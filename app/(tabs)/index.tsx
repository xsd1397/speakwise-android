        ...
        <ScrollView ref={scrollRef} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          ...
          <View style={[styles.card, styles.aiCard]}>
            ...
            <View style={styles.inputRow}>
              <TextInput value={input} onChangeText={setInput} onSubmitEditing={sendReply} placeholder="输入英文回复" placeholderTextColor={COLORS.muted} style={styles.input} accessibilityLabel="输入 AI 对话回复" />
              <Pressable onPress={sendReply} style={styles.send} accessibilityLabel="发送 AI 对话回复">
            ...
            <View style={styles.actions}>
              ...
              <Pressable onPress={() => setShowAiHints((value) => !value)} style={styles.action}>
                <Text style={styles.actionText}>{showAiHints ? "收起回复提示" : "回复提示"}</Text>
              </Pressable>
            </View>
            {showAiHints && (
              <View style={styles.hints}>
                {hintTexts.map((hint, index) => (
                  <Pressable key={hint.text} onPress={() => { setInput(hint.text); speak(hint.text, "Mia"); }} style={styles.hint}>
                    <Text style={styles.helper}>提示 {index + 1}：{hint.text}</Text>
