...
const COLORS = { bg: "#0B0C0F", panel: "#111317", panel2: "#151820", border: "#3A3D45", text: "#F2F3F5", muted: "#9AA2B4", blue: "#2F6BEB", blueSoft: "#1D3D86", orange: "#FFB15C", correctionBg: "#1A233A", correctionBorder: "#4A6BEB", success: "#73E2A8" };
...
function buildReplyHints(reply: string): ReplyHint[] {
  ...
  if (/\b(what|where|when|why|how|could|would|do you|are you|have you)\b|[?]/i.test(source)) {
    return [
      { text: `To answer your question about "${quoted}" I would like to share my own experience.`, translation: `关于"${quoted}"这个话题，我想分享一下我自己的经验。` },
      { text: `That is an interesting point about "${quoted}" could you tell me more about it?`, translation: `关于"${quoted}"这一点很有意思，你能告诉我更多细节吗？` },
    ];
  }
  ...
  return [
    { text: `I agree with your point about "${quoted}" and I can give an example.`, translation: `我同意你关于"${quoted}"的观点，我可以举个例子。` },
    { text: `I would like to add one detail about "${quoted}" from my own experience.`, translation: `关于"${quoted}"，我想根据我自己的经验补充一个细节。` },
  ];
}
...
export default function PracticeScreen() {
  ...
  useEffect(() => { setDialogue([{ role: "assistant", text: target.text, translation: target.translation }]); setTranslated({}); }, [scene, level, target.text]);
  useEffect(() => { Speech.getAvailableVoicesAsync().then(setVoices).catch(() => setVoices([])); return () => { Speech.stop?.(); }; }, []);

  // --- 升级：单词点击逻辑 (引入 API 兜底翻译) ---
  useEffect(() => { 
