...
export function WordbookProvider({ children }: { children: ReactNode }) { const [words, setWords] = useState<SavedWord[]>([]); const toggleWord = (word: string, example: string, scene: string) => { const key = word.toLowerCase(); setWords((current) => current.some((item) => item.word === key) ? current.filter((item) => item.word !== key) : [...current, { word: key, ...getWordDefinition(word), example, scene }]); }; const value = useMemo(() => ({ words, toggleWord, hasWord: (word: string) => words.some((item) => item.word === word.toLowerCase()) }), [words]); return <WordbookContext.Provider value={value}>{children}</WordbookContext.Provider>; }
