...
async function callMutation<T>(procedure: string, input: unknown): Promise<T> {
  ...
  if (!baseUrl) {
    throw new Error("尚未配置后端地址，请在 app.config.ts 或 EXPO_PUBLIC_API_BASE_URL 中设置。");
  }
  ...
  if (!response.ok) {
    throw new Error(payload.error?.json?.message ?? `请求失败（${response.status}）`);
  }
...
export async function evaluateRecording(uri: string, targetSentence: string, mimeType = "audio/mp4") {
  ...
  return callMutation<EvaluationResult>("voice.evaluate", {
    audioBase64,
    mimeType,
    targetSentence,
    language: "en",
...
export async function transcribeRecording(uri: string, mimeType = "audio/mp4") {
  if (!uri) throw new Error("没有找到真实录音文件，请重新录制。");
  const audioBase64 = await FileSystem.readAsStringAsync(uri, { encoding: FileSystem.EncodingType.Base64 });
  if (!audioBase64) throw new Error("录音文件为空，请重新录制。");
  return callMutation<{ text: string; duration?: number }>("voice.transcribe", {
    audioBase64,
    mimeType,
    language: "en",
  });
