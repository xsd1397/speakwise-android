...
async function callMutation<T>(procedure: string, input: unknown): Promise<T> { const baseUrl = getApiBaseUrl(); if (!baseUrl) throw new Error("尚未配置后端地址，请在 app.config.ts 或 EXPO_PUBLIC_API_BASE_URL 中设置。"); const response = await fetch(`${baseUrl}/api/trpc/${procedure}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ json: input }) }); const payload = await response.json() as { result?: { data?: { json?: T } }; error?: { json?: { message?: string } }; }; if (!response.ok) throw new Error(payload.error?.json?.message ?? `请求失败（${response.status}）`); const result = payload.result?.data?.json; if (result === undefined) throw new Error("服务端返回了无法识别的数据。"); return result; }
export async function replyToDialogue(input: { level: "beginner" | "intermediate" | "advanced"; scene: "greetings" | "travel" | "business" | "housing" | "medical" | "banking" | "shopping" | "transit" | "government" | "school" | "restaurant" | "emergency"; history: DialogueMessage[]; userMessage: string }) { return callMutation<{ reply: string; translation?: string; correction?: string; correctedEnglish?: string }>("dialogue.reply", input); }
export async function translateToChinese(text: string) { const value = text.trim(); if (!value) return value; try { const response = await fetch(`https://api.mymemory.translated.net/get?q=${encodeURIComponent(value)}&langpair=en|zh-CN`); const payload = await response.json() as { responseData?: { translatedText?: string } }; return payload.responseData?.translatedText?.trim() || "中文翻译暂不可用"; } catch { return "中文翻译暂不可用"; } }
export async function translateToEnglish(text: string) { const value = text.trim(); if (!value) return value; try { const response = await fetch(`https://api.mymemory.translated.net/get?q=${encodeURIComponent(value)}&langpair=zh-CN|en`); const payload = await response.json() as { responseData?: { translatedText?: string } }; const translated = payload.responseData?.translatedText?.trim(); return translated || value; } catch { return value; } }
